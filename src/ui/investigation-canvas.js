import { applyRepulsion, DEFAULT_FORCE_SETTINGS, forceIterationLimit, normalizeForceSettings, stabilizeForceNode } from "../graph/force-layout.js";
...
export class InvestigationCanvas {
  constructor(canvas, tooltip, { onSelectionChange, onEventOpen, forceSettings, saveForceSettings = () => {} } = {}) {
    this.canvas = canvas;
    this.document = canvas.ownerDocument;
    this.window = this.document.defaultView;
    this.listeners = [];
    this.destroyed = false;
    this.saveForceSettings = saveForceSettings;
  ...
  bind() {
    ...
    this.listen(this.canvas, "pointercancel", () => { this.pan = null; this.startSimulation(); });
    this.listen(this.canvas, "pointerleave", () => { if (!this.pan) this.showTooltip(null); });
  ...
  resize() {
    ...
    this.pixelRatio = Math.min(2, this.window.devicePixelRatio || 1);
  ...
  persistForceSettings() {
    try { this.saveForceSettings({ ...this.forceSettings }); }
  ...
  scheduleSimulation() {
    ...
    this.frame = this.window.requestAnimationFrame(() => {
      this.frame = null;
      const deadline = this.window.performance.now() + 10;
      const batch = this.nodes.length < 600 ? 8 : this.nodes.length < 2_500 ? 4 : 2;
      for (let step = 0; step < batch && this.alpha > .012 && this.window.performance.now() < deadline; step += 1) this.simulationStep();
      this.draw();
  ...
  showTooltip(node, clientX, clientY) {
    ...
    const title = this.document.createElement("strong"); title.textContent = node.kind === "event" ? node.label : `${node.typeLabel}: ${node.label}`;
    const details = this.document.createElement("span"); details.textContent = node.kind === "event"
    ...
    this.tooltip.style.left = `${Math.max(8, Math.min(this.window.innerWidth - 330, clientX + 14))}px`;
    this.tooltip.style.top = `${Math.max(8, Math.min(this.window.innerHeight - 130, clientY + 14))}px`;
  }
  ...
  draw() {
    ...
    const dark = this.window.matchMedia("(prefers-color-scheme: dark)").matches;
    context.lineWidth = 1;
