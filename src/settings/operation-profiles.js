...
export const OPERATION_FIELDS = Object.freeze([
  ['name', 'Название профиля'], ['sourceField', 'Поле источника'], ['sourceValues', 'Значения источника (через запятую)'],
  ...
  ['recordId', 'Уникальный ID события'], ['time', 'Время события'],
]);
const fields = new Set(['sourceField', 'eventField', 'operationField', 'host', 'pid', 'guid', 'target', 'targetPort', 'targetPid', 'protocol', 'recordId', 'time']);
...
export function normalizeOperationProfiles(profiles) {
  ...
  return profiles.map(profile => {
    ...
    const result = { id: profile.id, category: profile.category, platform: profile.platform, enabled: Boolean(profile.enabled) };
    ...
    if (Boolean(result.operationField) !== Boolean(result.operationValues)) throw new Error('Задайте и поле, и значения операции');
...
export function operationFact(event, profile, read, parseTime) {
  ...
  const port = get('targetPort'), pid = get('targetPid'), protocol = get('protocol');
  return { id: String(get('recordId') ?? ''), host: String(get('host') ?? ''), pid: get('pid'), guid: get('guid'),
    time: parseTime(get('time')), objectKey: target ? JSON.stringify([target, port, pid, protocol]) : '',
    label: [target, port && `порт ${port}`, pid && `PID ${pid}`, protocol].filter(Boolean).join(' · '),
    operation: String(get('eventField')), raw: event, profile: profile.name, recordField: profile.recordId, timeField: profile.time };
}
